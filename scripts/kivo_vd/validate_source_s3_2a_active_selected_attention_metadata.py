...
"""Validate the Phase S3.2A active selected-attention metadata report."""
...
SCHEMA = "kivo_source_s3_2a_active_selected_attention_metadata_v1"
POLICY_NAME = "active_sketch_selected_attention_metadata"
SELECTION_POLICY_NAME = "slot_coverage_recency_proxy"
ACTIVE_FILTER_MODE = "alias_excluded_blocks_to_recent_selected"


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Phase S3.2A active selected-attention metadata."
    )
    parser.add_argument("--input-json", required=True)
    parser.add_argument("--events-jsonl", required=True)
    parser.add_argument(
        "--output-json",
        default="outputs/kivo_vd/runs/source_s3_2a_active_selected_attention_metadata_validation.json",
    )
    parser.add_argument(
        "--output-md",
        default="outputs/kivo_vd/runs/source_s3_2a_active_selected_attention_metadata_validation.md",
    )
...
def load_report(path: str | Path) -> dict[str, Any]:
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"S3.2A input is missing: {input_path}")
...
def load_events(path: str | Path) -> list[dict[str, Any]]:
    input_path = Path(path)
    if not input_path.exists():
        raise FileNotFoundError(f"S3.2A events input is missing: {input_path}")
    events = []
...
def validate_events(events: list[dict[str, Any]]) -> list[str]:
    ...
    for index, event in enumerate(events):
        ...
        if mutation_applied:
            if event.get("mutation_attempted") is not True:
                errors.append(
                    f"event {index} applied mutation without mutation_attempted"
                )
            ...
            if not isinstance(alias_target, int) or alias_target < 0:
                errors.append(f"event {index} has invalid alias target")
            if int(event.get("aliased_block_count", 0) or 0) <= 0:
                errors.append(
                    f"event {index} must alias at least one block when applied"
                )
            for pair_index, pair in enumerate(
                event.get("alias_pairs_sample", [])
            ):
                if not isinstance(pair, dict):
                    errors.append(
                        f"event {index} alias pair {pair_index} must be an object"
                    )
                    continue
                excluded_id = pair.get("excluded_block_id")
                pair_target = pair.get("alias_target_block_id")
                if not isinstance(excluded_id, int) or excluded_id < 0:
                    errors.append(
                        f"event {index} alias pair {pair_index} has invalid "
                        "excluded block id"
                    )
                if pair_target != alias_target:
                    errors.append(
                        f"event {index} alias pair {pair_index} target mismatch"
                    )
        elif event.get("active_routing") is True:
            errors.append(
                f"event {index} claims active_routing without mutation_applied"
            )
...
def validate_active_metadata(
    ...
    if not s3_events:
        errors.append("total_s3_2a_events must be > 0")
    ...
    if report.get("s3_2a_active_metadata_passed") is not True:
        errors.append("s3_2a_active_metadata_passed must be true")
    ...
    return {
        ...
        "total_s3_2a_events": len(s3_events),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            "",
            "This validates active metadata control only. It does not prove "
            "memory reduction, latency improvement, quality preservation, "
            "or final selected attention.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = validate_active_metadata(
        ...
        json.dumps(
            {
                ...
                "total_s3_2a_events": report["total_s3_2a_events"],
