...
"""Run the Phase S3.1A shadow selected-attention metadata probe."""
...
DEFAULT_PROMPTS = [
    ...
    (
        "Kivo source experiments can plan selected attention metadata "
        "without changing runtime behavior. Kivo source experiments can "
        "plan selected attention metadata without changing runtime behavior."
    ),
]
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Phase S3.1A shadow selected-attention planning."
    )
    ...
    parser.add_argument(
        "--output-json",
        default="outputs/kivo_vd/runs/source_s3_1a_shadow_selected_attention_metadata.json",
    )
    parser.add_argument(
        "--output-md",
        default="outputs/kivo_vd/runs/source_s3_1a_shadow_selected_attention_metadata.md",
    )
    parser.add_argument(
        "--events-jsonl",
        default="outputs/kivo_vd/runs/source_s3_1a_shadow_selected_attention_metadata_events.jsonl",
    )
    ...
    *,
    ...
    return {
        ...
        **event_counts,
        ...
        "s3_1a_shadow_plan_passed": s3_1a_shadow_plan_passed,
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        f"- Passed: `{report['s3_1a_shadow_plan_passed']}`",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {
                "passed": report["s3_1a_shadow_plan_passed"],
    ...
    failed = failed or report["total_s3_1a_events"] <= 0
