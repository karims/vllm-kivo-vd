...
"""Validate the Phase S3.1B shadow sketch-selected attention report."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Phase S3.1B shadow sketch-selected attention JSON."
    )
    parser.add_argument("--input-json", required=True)
    parser.add_argument("--events-jsonl", required=True)
    parser.add_argument(
        "--output-json",
        default="outputs/kivo_vd/runs/source_s3_1b_shadow_sketch_selected_attention_metadata_validation.json",
    )
    parser.add_argument(
        "--output-md",
        default="outputs/kivo_vd/runs/source_s3_1b_shadow_sketch_selected_attention_metadata_validation.md",
    )
...
def load_report(path: str | Path) -> dict[str, Any]:
    ...
    if not input_path.exists():
        raise FileNotFoundError(f"S3.1B input is missing: {input_path}")
...
def load_events(path: str | Path) -> list[dict[str, Any]]:
    input_path = Path(path)
    if not input_path.exists():
        raise FileNotFoundError(f"S3.1B events input is missing: {input_path}")
    events = []
...
def validate_events(events: list[dict[str, Any]]) -> list[str]:
    ...
    for index, event in enumerate(events):
        ...
        if event.get("policy_name") != "shadow_sketch_selected_attention_metadata":
            errors.append(f"event {index} has unexpected policy_name")
        if event.get("selection_policy_name") != "slot_coverage_recency_proxy":
...
def validate_shadow_sketch_plan(
    report: dict[str, Any],
    events: list[dict[str, Any]],
) -> dict[str, Any]:
    errors: list[str] = []
    ...
    required = [
        ...
        "selected_attention_claim_allowed",
        "performance_claim_allowed",
        "s3_1b_shadow_sketch_plan_passed",
        "prompt_results",
    ]
    ...
    if total_s3_events <= 0:
        errors.append("total_s3_1b_events must be > 0")
    ...
    if report.get("s3_1b_shadow_sketch_plan_passed") is not True:
        errors.append("s3_1b_shadow_sketch_plan_passed must be true")
    ...
    return {
        ...
        "total_s3_1b_events": total_s3_events,
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Kivo-VD Phase S3.1B Shadow Sketch Selected-Attention Validation",
        "",
    ...
    lines.extend(
        [
            "",
            "This validation confirms shadow proxy scoring only. It does "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = validate_shadow_sketch_plan(
