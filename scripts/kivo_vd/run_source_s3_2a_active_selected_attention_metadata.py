...
"""Run the Phase S3.2A active selected-attention metadata probe."""
...
DEFAULT_PROMPTS = [
    ...
    (
        "Kivo source experiments can actively alter selected attention "
        "metadata. Kivo source experiments can actively alter selected "
        "attention metadata."
    ),
]
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Phase S3.2A active selected-attention metadata filtering."
    )
    ...
    parser.add_argument(
        "--output-json",
        default="outputs/kivo_vd/runs/source_s3_2a_active_selected_attention_metadata.json",
    )
    parser.add_argument(
        "--output-md",
        default="outputs/kivo_vd/runs/source_s3_2a_active_selected_attention_metadata.md",
    )
    parser.add_argument(
        "--events-jsonl",
        default="outputs/kivo_vd/runs/source_s3_2a_active_selected_attention_metadata_events.jsonl",
    )
...
def _set_active_env(event_path: str | Path, args: argparse.Namespace) -> None:
    ...
    os.environ["KIVO_SOURCE_COVERAGE_WEIGHT"] = str(args.coverage_weight)
    os.environ["KIVO_SOURCE_RECENCY_WEIGHT"] = str(args.recency_weight)
...
def summarize_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "max_visible_block_count": _max_int(
            filtered, "visible_block_count_estimate"
        ),
        ...
        "max_aliased_block_count": _max_int(filtered, "aliased_block_count"),
    ...
    return {
        ...
        **event_counts,
        ...
        "max_aliased_block_count": max(
            (result["max_aliased_block_count"] for result in prompt_results),
            default=0,
        ),
        ...
        "s3_2a_active_metadata_passed": s3_2a_active_metadata_passed,
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        f"- Passed: `{report['s3_2a_active_metadata_passed']}`",
        ...
        f"- Max aliased block count: `{report['max_aliased_block_count']}`",
    ...
    for item in report["prompt_results"]:
        lines.extend(
            [
                ...
                f"- Active routing events: `{item['active_routing_event_count']}`",
    ...
    lines.extend(
        [
            "## Boundary",
            "",
            "- This aliases excluded block IDs in a cloned metadata block table.",
            "- It does not mutate scheduler state or KV allocation.",
            "- It does not prove memory reduction, latency improvement, or quality.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {
                "passed": report["s3_2a_active_metadata_passed"],
    ...
    return 0 if report["s3_2a_active_metadata_passed"] else 1
