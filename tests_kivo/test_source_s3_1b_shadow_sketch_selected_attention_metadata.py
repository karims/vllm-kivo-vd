...
from scripts.kivo_vd import (
    run_source_s3_1b_shadow_sketch_selected_attention_metadata as runner,
)
from scripts.kivo_vd import (
    validate_source_s3_1b_shadow_sketch_selected_attention_metadata as validator,
)
...
def _good_report() -> dict[str, object]:
    ...
    return {
        ...
        "selected_attention_claim_allowed": False,
        "performance_claim_allowed": False,
        "s3_1b_shadow_sketch_plan_passed": True,
        "prompt_results": prompt_results,
        "events_jsonl": "events.jsonl",
    }


def test_parse_args_includes_proxy_scoring_flags():
    ...
    assert args.coverage_weight == 0.7
    assert args.recency_weight == 0.3


def test_summarize_records_tracks_exclusion_and_fallback_stats():
    ...
    assert summary["fallback_event_count"] == 1
    assert summary["max_visible_block_count"] == 3


def test_validate_shadow_sketch_plan_passes_for_mixed_prompt_behavior():
    report = _good_report()
    events = [
        _good_event(
            visible_block_count_estimate=1,
    ...
    result = validator.validate_shadow_sketch_plan(report, events)
    assert result["validation_passed"] is True
    assert result["total_raw_events"] == 3
    assert result["total_s3_1b_events"] == 3
    assert result["ignored_non_s3_events"] == 0


def test_validate_shadow_sketch_plan_filters_mixed_schema_events():
    report = _good_report()
    events = [
        {
            "schema_version": "kivo_source_s3_1a_shadow_selected_attention_metadata_v1",
    ...
    assert result["total_s3_1b_events"] == 1
    assert result["ignored_non_s3_events"] == 1


def test_validate_shadow_sketch_plan_rejects_when_no_shadow_events_exist():
    report = _good_report()
    result = validator.validate_shadow_sketch_plan(
        report,
        [
            {
                "schema_version": "kivo_source_s3_1a_shadow_selected_attention_metadata_v1",
    ...
    assert any("total_s3_1b_events must be > 0" in e for e in result["errors"])


def test_validate_shadow_sketch_plan_rejects_missing_exclusion_for_eligible_events():
    report = _good_report()
    events = [
        _good_event(
            visible_block_count_estimate=3,
...
def test_validate_shadow_sketch_plan_accepts_fallback_events():
    report = _good_report()
    events = [
        _good_event(
            visible_block_count_estimate=2,
...
def test_validate_shadow_sketch_plan_rejects_runtime_claims():
    report = _good_report()
